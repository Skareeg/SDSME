...
namespace AB_API
{
    public class AB
    {
        ...
        {
            System.IO.BinaryReader read = new System.IO.BinaryReader(File.OpenRead(filePath));
            ...
            for (int i = 0; i < fileCount / 2; i++)
            {
                ...
                {
                    System.IO.BinaryWriter write = new System.IO.BinaryWriter(File.Create(folderPath + "\\" + "header" + "\\" + i.ToString("D4")));
            ...
            for (int i = fileCount / 2; i < fileCount; i++)
            {
                ...
                System.IO.BinaryWriter write = new System.IO.BinaryWriter(File.Create(folderPath + "\\" + "model" + "\\" + (i - fileCount / 2).ToString("D4")));
                int size = read.ReadInt32();
                read.BaseStream.Position = offsets[i];
        ...
        {
            System.IO.BinaryWriter write = new System.IO.BinaryWriter(File.Create(filePath));
            ...
            for (int i = 0; i < count; i++)
            {
                System.IO.BinaryReader read = new System.IO.BinaryReader(File.OpenRead(folderPath + "\\" + "header" + "\\" + i.ToString("D4")));
                write.Write(read.ReadBytes((int)new FileInfo(folderPath + "\\" + "header" + "\\" + i.ToString("D4")).Length));
                read.Close();
            }
            for (int i = 0; i < count; i++)
            {
                System.IO.BinaryReader read = new System.IO.BinaryReader(File.OpenRead(folderPath + "\\" + "model" + "\\" + i.ToString("D4")));
